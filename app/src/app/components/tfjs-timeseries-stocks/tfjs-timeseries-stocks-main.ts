...
export class TfjsTimeseriesStocksMain {
  ...
  async demo_1_fetchData(ticker:string, apikey:string, data_temporal_resolutions:string, period:Period){
    ...
    return new Promise((resolve, reject) => {
      ...
      if(data_temporal_resolutions == 'Daily'){
        ...
        // requestUrl = "https://www.alphavantage.co/query?function=FX_DAILY&from_symbol=XAU&to_symbol=USD&apikey=X3QXPWK0AA0ILVGC";
        // requestUrl = "https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=XAUUSD&apikey=X3QXPWK0AA0ILVGC"

        // requestUrl = `http://app-normalize-data:3000/static/${ticker}.json`;
      
        
        // requestUrl = `http://localhost:3000/static/${ticker}.json`;
        requestUrl = `http://localhost:3002/stock/${ticker}`;
      }else{
      ...
      this.httpClient.post(requestUrl, {
        ...
        data  => {
          ...
          if(data.hasOwnProperty('data')) {
            let symbol = 'Symbol';
            let last_refreshed = 'Last Refreshed';

            let data_raw = [];

            data['data'].map((o,i) => {
              data_raw.push({ timestamp: o.dt, price: parseFloat(o.price) });
            });
            data_raw.reverse();
            let message = "Symbol: " + symbol + " (last refreshed " + last_refreshed + ")";
  ...
  async saveModel(model) {
    const saveResult = await model.save('http://localhost:3000/model');
  ...
  async loadModel() {
    ...
    const hist = {};
    return { model: tf.sequential(model), stats: hist };
  }
